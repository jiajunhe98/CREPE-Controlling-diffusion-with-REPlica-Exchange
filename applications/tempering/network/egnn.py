...
class EGNN_dynamics_AD6(nn.Module):
...
class EGNN_dynamics_AD2_cat(nn.Module):
    def __init__(self, n_particles, n_dimension,hidden_nf=64, device='cpu',
            act_fn=torch.nn.SiLU(), n_layers=4, recurrent=True, attention=False,
                 condition_time=True, tanh=False, mode='egnn_dynamics', agg='sum', data_sigma=1):
        super().__init__()
        self.mode = mode
        ...
        atom_types[[19, 20, 21]] = 20
        atom_types[[11, 12, 13]] = 12
        h_initial = torch.nn.functional.one_hot(torch.tensor(atom_types, dtype=torch.long)).to(device)
