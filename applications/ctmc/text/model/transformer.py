...
class SEDD(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, indices, sigma, lengths=None):
        x = self.vocab_embed(indices)
        c = F.silu(self.sigma_map(sigma))

        rotary_cos_sin = self.rotary_emb(x, lengths=lengths)
...
class DeftSEDD(nn.Module, PyTorchModelHubMixin):
    sedd: SEDD

    def __init__(self, sedd):
        super().__init__()
        self.sedd = sedd

    def forward(self, x, x0, cond, sigma):
        network_input = torch.concat([x, x0, cond], axis=-1)
        B, V = x.shape
        return self.sedd(
            network_input, sigma, lengths=[x.shape[-1], x0.shape[-1], cond.shape[-1]]
        )[:, :V]


class FinetuneSEDD(nn.Module, PyTorchModelHubMixin):
    sedd: SEDD
    ...
    def forward(self, x, cond, sigma):
        network_input = torch.concat([x, cond], axis=-1)
        B, V = x.shape
        return self.sedd(network_input, sigma, lengths=[x.shape[-1], cond.shape[-1]])[
            :, :V
        ]
