...
class ResBlock(nn.Module):
    def __init__(
        ...
        if class_embed_dim is not None:
            self.cls_linear = nn.Linear(class_embed_dim, in_ch)

    def forward(
        self,
        x,  # ["batch", "in_ch", "H", "W"]
        temb=None,  #  ["batch", "temb_dim"]
        cls_embed=None,  # ["batch", "class_embed_dim"]
        mask=None,  # ["batch"]  - binary mask (0 or 1) for conditional generation
    ):
        assert x.shape[1] == self.in_ch

        # Apply class conditioning if both cls_embed and mask are provided
        if cls_embed is not None and mask is not None:
            cls_embed = self.cls_linear(cls_embed)
            # Apply cls_embed only where mask is 1
            x = x + cls_embed[:, :, None, None] * mask[:, None, None, None]
...
class UNet(nn.Module):
    def __init__(
        ...
        self.output_channels = 2 * output_channels
    ...
    def _do_downsampling(self, h, hs, temb, cls_embed=None, mask=None):
        m_idx = 0
        for scale_count in range(self.num_scales):
            for res_count in range(self.num_res_blocks):
                if isinstance(self.downsampling_modules[m_idx], ResBlock):
                    h = self.downsampling_modules[m_idx](h, temb, cls_embed, mask)
                else:
    ...
    def _do_middle(self, h, temb, cls_embed=None, mask=None):
        m_idx = 0
        if isinstance(self.middle_modules[m_idx], ResBlock):
            h = self.middle_modules[m_idx](h, temb, cls_embed, mask)
        else:
    ...
    def _do_upsampling(self, h, hs, temb, cls_embed=None, mask=None):
        m_idx = 0
        for scale_count in reversed(range(self.num_scales)):
            for res_count in range(self.num_res_blocks + 1):
                if isinstance(self.upsampling_modules[m_idx], ResBlock):
                    h = self.upsampling_modules[m_idx](
                        torch.cat([h, hs.pop()], dim=1), temb, cls_embed, mask
                    )
                else:
                    h = self.upsampling_modules[m_idx](
                ...
                if scale_count == self.scale_count_to_put_attn:
                    if isinstance(self.upsampling_modules[m_idx], ResBlock):
                        h = self.upsampling_modules[m_idx](h, temb, cls_embed, mask)
                    else:
    ...
    def _logistic_output_res(
        self,
        h,  #  ["B", "twoC", "H", "W"]
        centered_x_in,  # ["B", "C_input", "H", "W"]
    ):
        B, twoC, H, W = h.shape
        C_input = centered_x_in.shape[1]  # Use actual input channels
        h[:, 0:C_input, :, :] = torch.tanh(centered_x_in + h[:, 0:C_input, :, :])
        return h
    ...
    def forward(
        ...
        x,  # ["B", "C", "H", "W"]
        timesteps=None,  # ["B"]
        cls_embed=None,  # ["B", "class_embed_dim"]
        mask=None,  # ["B"]
    ):
        ...
        h, hs = self._do_downsampling(h, hs, temb, cls_embed, mask)

        h = self._do_middle(h, temb, cls_embed, mask)

        h = self._do_upsampling(h, hs, temb, cls_embed, mask)
