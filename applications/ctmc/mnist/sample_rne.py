#!/usr/bin/env python3
"""
Sample from a CFG model trained on MNIST using Radon-Nikodym Estimation (RNE).

Usage:
    python sample_rne.py --model-path checkpoints/trained_model.pt
    python sample_rne.py --model-path checkpoints/trained_model.pt --num-particles 128 --smc-temperature 2.0
"""
...
def main():
    parser = argparse.ArgumentParser(description="RNE sampling for MNIST")
    ...
    parser.add_argument("--output-dir", default="./outputs/rne")
    ...
    print("=== MNIST RNE Sampling ===")
    print(f"Using {args.num_particles} particles per RNE run")
    print(f"Running {args.num_runs} RNE runs")
    ...
    # Create RNE sampler
    rne_sampler = sampler.get_cfg_rne_sampler(
        graph=graph_module,
        noise=noise_module,
    ...
    for run_idx in range(args.num_runs):
        print(f"\nRNE Run {run_idx + 1}/{args.num_runs}")
        print(f"Running {args.num_iterations} RNE iterations per run")

        class_labels = target_class_tensor[run_idx].repeat(args.num_particles)
        current_class = target_class_tensor[run_idx].item()

        for rne_iter in range(args.num_iterations):
            print(f"  RNE Iteration {rne_iter + 1}/{args.num_iterations}")
            ...
            all_rne_samples.append(samples_np)
            ...
            grid_path = os.path.join(
                args.output_dir,
                f"rne_run{run_idx}_iter{rne_iter}_class{int(current_class)}.png",
            )
    ...
    # Save all RNE samples as numpy array
    if len(all_rne_samples) > 0:
        all_samples_array = np.stack(all_rne_samples, axis=0)
        total_samples = all_samples_array.reshape(-1, 784)
        total_samples_reshaped = total_samples.reshape(-1, 1, 28, 28)
        samples_save_path = os.path.join(args.output_dir, "all_rne_samples.npy")
        np.save(samples_save_path, total_samples_reshaped)
        print(f"\nAll RNE samples saved to: {samples_save_path}")
